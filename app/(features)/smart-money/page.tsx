        ...
        {FILTERS.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors border cursor-pointer ${
              filter === f ? 'bg-[#4A90C4]/10 text-[#4A90C4] border-[#4A90C4]/20' : 'text-muted-foreground border-[var(--border)] hover:text-foreground hover:bg-card'
            }`}>
            {f}
          </button>
        ))}
          ...
          <div key={entry.id} className="rounded-xl border bg-card p-4 hover:border-[var(--ring)] transition-colors cursor-pointer">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-[#4A90C4]/10 text-[#4A90C4]">{entry.archetype}</span>
              </div>
              <Bird size={16} weight="fill" className="text-[#4A90C4]" />
            </div>
            <p className="text-[14px] mb-1.5">
              <span className="text-muted-foreground">{entry.action}</span>{' '}
              <span className="font-semibold text-[#4A90C4]">{entry.amount}</span>
            </p>
            <div className="rounded-lg p-2.5 mt-1"
              ...
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] uppercase tracking-[1.5px] font-semibold text-[#4A90C4]">Pelican</span>
              </div>
