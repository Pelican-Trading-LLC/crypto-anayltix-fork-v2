...
export function WalletEmptyState({ onSelectWallet }: Props) {
  return (
    <div className="max-w-[800px] mx-auto text-center py-8">
      <Bird size={56} weight="thin" className="mx-auto mb-4 text-[#1DA1C4]/40" />
        ...
        {FEATURED_WALLETS.map((fw, i) => {
          ...
          return (
            <button key={fw.address} onClick={() => onSelectWallet(fw.address)}
              className="rounded-xl border bg-card p-4 hover:border-[#1DA1C4]/30 hover:translate-y-[-1px] transition-all text-left cursor-pointer group">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#1DA1C4]/10 flex items-center justify-center">
                  <Icon size={16} className="text-[#1DA1C4]" />
                </div>
                <div>
                  <span className="text-[12px] text-muted-foreground">{fw.label}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] font-medium group-hover:text-[#1DA1C4] transition-colors">{fw.archetype}</span>
                    <span className="font-mono text-[11px] text-[#1DA1C4]">{fw.stat}</span>
                  </div>
                </div>
              </div>
            </button>
