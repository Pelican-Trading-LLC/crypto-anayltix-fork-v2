...
export function usePanelLayout() {
  ...
  const handleSidebarToggle = useCallback(() => {
    setSidebarCollapsed((prev) => {
      const next = !prev
      localStorage.setItem("pelican_sidebar_collapsed", String(next))
      return next
    })
  }, [])
  ...
  const handleTradingPanelToggle = useCallback(() => {
    setTradingPanelCollapsed((prev) => {
      const next = !prev
      localStorage.setItem("pelican_trading_panel_collapsed", String(next))
      return next
    })
  }, [])
  ...
  const handleResizeStart = useCallback((e: ReactMouseEvent) => {
    ...
    const onMouseUp = () => {
      ...
      setPanelWidth((width) => {
        localStorage.setItem("pelican_trading_panel_width", String(width))
        return width
      })
    }
  ...
  useEffect(() => {
    ...
    const savedPanelWidth = localStorage.getItem("pelican_trading_panel_width")
    if (savedPanelWidth) {
